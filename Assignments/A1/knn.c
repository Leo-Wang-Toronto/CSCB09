...
Image *load_image(char *filename) {
    ...
    image->data = calloc(sizeof(unsigned char), image->sx * image->sy);
...
Dataset *init_dataset(int size) {
    Dataset *dataset = (Dataset *)calloc(sizeof(Dataset), 1);
    if (dataset == NULL) {
        perror("callac");
        exit(1);
    }
    ...
    dataset->labels = calloc(sizeof(unsigned char), size);
    if (dataset->labels == NULL) {
        perror("callac");
        exit(1);
    }
    
    dataset->images = calloc(sizeof(Image), size);
