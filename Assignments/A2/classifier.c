...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: ./classifier training_file testing_file\n");
        exit(1);
    }
    ...
    DTNode *tree = make_dec_tree(training);
    ...
    {
        if (dec_tree_classify(tree, &testing->images[i]) == testing->labels[i]) {
            total_correct++;
        }
    }
